...
$("#selectButt").click(function (){
    location.href="/sybida/teacher/ClassstudentOfferForTeacher.html?pageNum=1&pageSize="+pageSize+"&classNum="+classNum
})
                        ...
                        if (i%2==0){
                            html +=`<tr class="warning"><td style="width: 80px;"><input type="checkbox" name="optionAll" value="${le[i].offerId}"></td>
                                      ...
                                      <td><button class="layui-btn layui-btn-xs" lay-event="edit" 
                                         date-offerId ="${le[i].offerId}" >查看详情</button>
                                      </td>
                                  
                                      <td>
                                         <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del" data-toggle="modal"
                                         data-id="${le[i].offerId}" data-name="${le[i].offerCompany}" data-target="#exampleModal" >删除</a>
                                      </td>
...
function pageSelect(data){
    ...
    if(pageNum==1){
        ...
        html+=`<li><a href="/sybida/teacher/ClassstudentOfferForTeacher.html?pageNum=${pageNum-1}&pageSize=${pageSize}&classNum=${classNum}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>`
    }
    for(var i=data.navigateFirstPage;i<=data.navigateLastPage;i++){
        if(pageNum==i){
            html+=`<li class="active"><a href="#">${i}</a></li>`
        }else{
            html+=`<li><a href="/sybida/teacher/ClassstudentOfferForTeacher.html?pageNum=${i}&pageSize=${pageSize}&classNum=${classNum}">${i}</a></li>`
        }
    }
    if((data.pages)<=pageNum){
        ...
        html+=`<li><a href="/sybida/teacher/ClassstudentOfferForTeacher.html?pageNum=${pa}&pageSize=${pageSize}&classNum=${classNum}" aria-label="Next"> <span aria-hidden="true">&raquo;</span></a></li>`
    }
...
$('#exampleModal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    modal.find('#messagetext').text('确认删除学生-' +name+'-的offer记录吗？')
})
