    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                ,{field:'studentNull1', title:'学生级别', width:'10%',totalRow: true}
                ,{field:'studyAspect', title:'简历方向', width:'10%', sort: true, totalRow: true}
                ,{field:'vitaeLevel', title:'可投递', width:'11%',}
                ,{field:'vitaeIsRead', title:'是否审阅', width:'11%',}
                ,{field:'studentPhone', title:'电话号码', width:'13%', totalRow: true}
                ,{field:'vitaeAlterTime', title:'修改时间', width:'18%', templet:'<div>{{ layui.util.toDateString(d.pressTime, "yyyy-MM-dd HH:mm:ss") }}</div>'}
            ]] , done: function(res, curr, count){

            $("[data-field='vitaeLevel']").children().each(function(){
                if($(this).text()==1){
                    $(this).html("<span style='color:black'>可投递</span>")
                }else if($(this).text()==0){
                    $(this).html("<span style='color:red'>不可投递</span>")
                }
            });
                $("[data-field='vitaeIsRead']").children().each(function(){
                    if($(this).text()==1){
                        $(this).html("<span style='color:black'>是</span>")
                    }else if($(this).text()==0){
                        $(this).html("<span style='color:red'>否</span>")
                    }
                });
            }
            // ,page: true
